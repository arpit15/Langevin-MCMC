...
void PyScene::camera_rays2(tensorhw3 &rayorg, tensorhw3 &raydir, tensorhw3 &rayminmax)
{
	// std::cout << "Num cores: " << NumSystemCores() << std::endl;
	const Camera *camera = _scene->camera.get();
			...
			[&](const Vector2i tile)
			{
				...
				for (size_t y = y0; y < y1; y++)
				{
					for (size_t x = x0; x < x1; x++)
					{
						...
						rayminmax(y, x, 0) = raySeg.minT;
						rayminmax(y, x, 1) = raySeg.maxT;
					}
				}
				reporter.Update(1);
			},
...
nb::tuple PyScene::ray_intersect_all2(
		tensorhw3 &rayorg,
		tensorhw3 &raydir,
		tensorhw3 &rayminmax)
{
	...
	ParallelFor(
			[&](const Vector2i tile)
			{
				...
				Vector3 currrayorg, currraydir, currnorm, currpos, currrayminmax;
				// // -----
				for (int i = 0; i < 3; ++i)
				{
					currrayorg(i) = rayorg(y, x, i);
				...
				currrayminmax(0) = rayminmax(y, x, 0);
				currrayminmax(1) = rayminmax(y, x, 1);
				currrayminmax(2) = 0; // arbitrarily chosen
				...
				// NANOLOG_INFO("({},{}), o:{}, d:{} n:{}, p:{}", y,x,
				// 	currrayorg.transpose(), currraydir.transpose(),
				// 	currnorm.transpose(), currpos.transpose());
		...
		Vector3 &pos)
{
	...
	auto tmpval = raySeg.ray.org.array().abs();
	float tmpval2 = std::max(tmpval(0), std::max(tmpval(1), tmpval(2)));
	raySeg.minT = rayminmax(0);
	raySeg.maxT = rayminmax(1);
	...
	if (Intersect(_scene.get(), time, raySeg, shapeInst))
	{
		...
		if (shapeInst.obj->Intersect(shapeInst.primID, time, raySeg, isect, shapeInst.st))
		{
			...
			// std::cout << "n:" << _normal.transpose()
			// 		<< ", pos:" << _pos.transpose() << std::endl;
			// NANOLOG_INFO("ID:{} o:{} , d:{}, n:{}, p:{}",
			//  shapeInst.obj->GetId(),
			//  raySeg.ray.org.transpose(), raySeg.ray.dir.transpose(),
			//  _normal.transpose(), _pos.transpose());
			hit = true;
		}
	}
